  ...
  const handleDeleteComment = async comment => {
    try {
      if (owner) {
        ...
        const path = window.location.pathname;
        const postId = path.match(/\d+/)[0];
        const res = await axios.delete(`/posts/${postId}/comments/${comment}`);
        ...
        {comments.length > 0
          ? comments.map(comment => (
              <div key={comment.id} className="d-flex mb-3">
                <div className="pr-2 flex-shrink-0" style={{ width: "50px" }}>
                  <img src={comment.avatar} className="rounded-circle w-100" />
                </div>
                <div className="pr-2 flex-grow-1">
                  ...
                  <small className="text-muted">
                    Posted{" "}
                    <Moment date={comment.created_at} format="DD/MM/YYYY" />
                  </small>
                </div>
                {owner ? (
                  <div className="pr-2">
